...
$requestsTable = "`{$requestsDb}`.`logistics_requests`";
...
if (!$tableExists) {
    echo '<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>Logistics - Items Requests</title><script src="https://cdn.tailwindcss.com"></script></head><body class="bg-gray-50 min-h-screen"><div class="max-w-3xl mx-auto p-6"><div class="bg-white rounded-xl shadow-md p-6"><h1 class="text-xl font-bold text-gray-800">Missing database table</h1><p class="text-gray-600 mt-2">The table <code class="bg-gray-100 px-1 rounded">logistics_requests</code> does not exist in the current database (<code class="bg-gray-100 px-1 rounded">' . htmlspecialchars((string)$conn->query('SELECT DATABASE()')->fetch_row()[0]) . '</code>).</p><p class="text-gray-600 mt-2">Import/run <code class="bg-gray-100 px-1 rounded">schema_training_requests.sql</code> in phpMyAdmin (make sure the correct database is selected).</p><div class="mt-4"><a class="inline-block px-4 py-2 bg-blue-600 text-white rounded" href="trainingprogram.php">Back to Training Programs</a></div></div></div></body></html>';
    exit;
}
 ...
 $ensureRequestSchema = function(mysqli $conn): void {
     ...
     try {
         if (!$tableHasColumn($conn, $requestsDb, 'logistics_requests', 'rejection_reason')) {
     ...
     try {
         if (!$tableHasColumn($conn, $requestsDb, 'logistics_requests', 'submission_no')) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'update_status' && isset($_POST['request_id'])) {
    ...
    try {
        $stmtLog = $conn->prepare("INSERT INTO {$requestLogsTable} (request_type, request_id, program_id, submission_no, old_status, new_status, reason) VALUES ('logistics', ?, ?, ?, NULLIF(?, ''), ?, NULLIF(?, ''))");
    ...
    if ($status === 'Rejected' && $programIdForLog > 0) {
        ...
        try {
            $stmtReqHold = $conn->prepare("UPDATE {$adminTable} SET status = 'ON HOLD' WHERE program_id = ? AND IFNULL(submission_no, 1) = ? AND status = 'Pending'");
            $stmtReqHold->bind_param('ii', $programIdForLog, $submissionNoForLog);
            $stmtReqHold->execute();
        } catch (Throwable $e) {
        }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['request_id'])) {
    $requestId = (int)$_POST['request_id'];
    $status = $_POST['status'] ?? 'Pending';
    $itemsRequested = $_POST['items_requested'] ?? null;

    $stmt = $conn->prepare("UPDATE {$requestsTable} SET status = ?, items_requested = ? WHERE id = ?");
    $stmt->bind_param('ssi', $status, $itemsRequested, $requestId);
    $stmt->execute();
...
if ($requestId) {
    $stmt = $conn->prepare("SELECT lr.*, tp.training_title, tp.training_type, tp.category, tp.participants_needed, tp.start_datetime, tp.end_datetime, tp.description, tp.status AS program_status
        FROM {$requestsTable} lr
        JOIN {$trainingProgramsTable} tp ON tp.id = lr.program_id
        WHERE lr.id = ?");
...
if (!empty($detail)) {
 ...
 if ($programId) {
     $stmt = $conn->prepare("SELECT lr.id AS request_id, lr.status, lr.created_at, tp.id AS program_id, tp.training_title, tp.start_datetime, tp.end_datetime, tp.participants_needed, tp.status AS program_status
         FROM {$requestsTable} lr
         JOIN {$trainingProgramsTable} tp ON tp.id = lr.program_id
         WHERE tp.id = ? AND lr.status IN ('Pending','ON HOLD') AND tp.status NOT IN ('Under Review','For Compliance') AND IFNULL(lr.submission_no, 1) = IFNULL(tp.submission_no, 1)
     ...
     $stmt = $conn->prepare("SELECT lr.id AS request_id, lr.status, lr.created_at, tp.id AS program_id, tp.training_title, tp.start_datetime, tp.end_datetime, tp.participants_needed, tp.status AS program_status
         FROM {$requestsTable} lr
         JOIN {$trainingProgramsTable} tp ON tp.id = lr.program_id
         WHERE lr.status IN ('Pending','ON HOLD') AND tp.status NOT IN ('Under Review','For Compliance') AND IFNULL(lr.submission_no, 1) = IFNULL(tp.submission_no, 1)
         ORDER BY lr.created_at DESC");
     $stmt->execute();
     $requests = $stmt->get_result()->fetch_all(MYSQLI_ASSOC);
 }
...
<div class="flex h-screen">
        ...
        <main class="container mx-auto px-4 py-6">
            <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-3 mb-6">
                <div>
                    ...
                    <p class="text-sm text-gray-500">Items requests created from Training Programs</p>
                </div>
                <div class="flex gap-2">
                    <a class="btn btn-outline btn-sm" href="trainingprogram.php">Back to Training Programs</a>
                    <a class="btn btn-outline btn-sm" href="request_logs.php?type=logistics">Logs</a>
                </div>
            </div>

            <?php if ($requestId && !empty($detail)) : ?>
                <dialog id="view-logistics-modal" class="modal" open>
                    <div class="modal-box w-11/12 max-w-3xl">
                        <div class="flex items-start justify-between gap-3">
                            ...
                            <button type="button" class="btn btn-ghost btn-sm" id="close-logistics-modal">✕</button>
                        </div>
                        ...
                        <div class="modal-action">
                            <form method="POST" class="inline" id="logistics-approve-form">
                            ...
                            <form method="POST" class="inline reject-form" id="logistics-reject-form">
                                <input type="hidden" name="action" value="update_status">
                                <input type="hidden" name="request_id" value="<?php echo (int)$detail['id']; ?>">
                                <input type="hidden" name="program_id" value="<?php echo (int)$detail['program_id']; ?>">
                            ...
                            <a class="btn btn-ghost" href="logistics.php<?php echo $programId ? '?program_id=' . (int)$programId : ''; ?>">Close</a>
                        </div>
                    </div>
            ...
            <div class="bg-white rounded-xl shadow-md overflow-hidden">
                    <div class="p-4 border-b border-gray-100">
                        <h2 class="font-semibold text-gray-700">Items Requests</h2>
                    </div>
                    <div>
                                        ...
                                        <tr data-request-id="<?php echo (int)$r['request_id']; ?>">
                                            ...
                                            <td data-label="Actions">
                                                <div class="flex flex-wrap gap-2">
                                                    <a class="btn btn-sm btn-outline" href="logistics.php?request_id=<?php echo (int)$r['request_id']; ?>">View</a>
 ...
 <script>
   (function () {
     const closeBtn = document.getElementById('close-logistics-modal');
     if (closeBtn) {
       closeBtn.addEventListener('click', function () {
         window.location.href = 'logistics.php<?php echo $programId ? '?program_id=' . (int)$programId : ''; ?>';
       });
     }
     ...
     const submitStatusAjax = async (form) => {
       ...
       try {
         const res = await fetch('logistics.php', { method: 'POST', body: fd, credentials: 'same-origin' });
