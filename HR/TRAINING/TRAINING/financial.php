...
if (!$tableExists) {
    echo '<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>Financial - Budget Requests</title><script src="https://cdn.tailwindcss.com"></script></head><body class="bg-gray-50 min-h-screen"><div class="max-w-3xl mx-auto p-6"><div class="bg-white rounded-xl shadow-md p-6"><h1 class="text-xl font-bold text-gray-800">Missing database table</h1><p class="text-gray-600 mt-2">The table <code class="bg-gray-100 px-1 rounded">financial_requests</code> does not exist in the current database (<code class="bg-gray-100 px-1 rounded">' . htmlspecialchars((string)$conn->query('SELECT DATABASE()')->fetch_row()[0]) . '</code>).</p><p class="text-gray-600 mt-2">Import/run <code class="bg-gray-100 px-1 rounded">schema_training_requests.sql</code> in phpMyAdmin (make sure the correct database is selected).</p><div class="mt-4"><a class="inline-block px-4 py-2 bg-blue-600 text-white rounded" href="trainingprogram.php">Back to Training Programs</a></div></div></div></body></html>';
    exit;
}
...
<head>
    ...
    <title>Financial - Budget Requests</title>
    ...
    <div class="flex flex-col flex-1 overflow-auto">
        ...
        <main class="container mx-auto px-4 py-6">
            <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-3 mb-6">
                <div>
                    <h1 class="text-2xl font-bold text-gray-800">Financial Requests</h1>
                    <p class="text-sm text-gray-500">Budget requests created from Training Programs</p>
                </div>
                <div class="flex gap-2">
                    <a class="btn btn-outline btn-sm" href="trainingprogram.php">Back to Training Programs</a>
                    <a class="btn btn-outline btn-sm" href="request_logs.php?type=financial">Logs</a>
                </div>
            </div>

            <?php if ($requestId && !empty($detail)) : ?>
                <dialog id="view-financial-modal" class="modal" open>
                    <div class="modal-box w-11/12 max-w-3xl">
                        <div class="flex items-start justify-between gap-3">
                            ...
                            <button type="button" class="btn btn-ghost btn-sm" id="close-financial-modal">✕</button>
                        </div>
                        ...
                        <div class="mt-4 grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                <div class="font-semibold text-gray-700">Budget Amount</div>
                                <div class="text-gray-900"><?php echo $detail['budget_amount'] !== null ? htmlspecialchars((string)$detail['budget_amount']) : 'N/A'; ?></div>
                            </div>
                            <div>
                        ...
                        <div class="modal-action">
                            <form method="POST" class="inline" id="financial-approve-form">
                            ...
                            <form method="POST" class="inline" id="financial-reject-form">
                                <input type="hidden" name="action" value="update_status">
                                <input type="hidden" name="request_id" value="<?php echo (int)$detail['id']; ?>">
                                <input type="hidden" name="program_id" value="<?php echo (int)$detail['program_id']; ?>">
                                <input type="hidden" name="status" value="Rejected">
                                <input type="hidden" name="reason" id="financial-reject-reason" value="">
                                <button type="button" class="btn btn-error" id="financial-reject-btn" <?php echo ((string)($detail['program_status'] ?? '') === 'ON HOLD') ? 'disabled' : ''; ?>>Reject</button>
                            </form>
                            <a class="btn btn-ghost" href="financial.php<?php echo $programId ? '?program_id=' . (int)$programId : ''; ?>">Close</a>
                        </div>
                    </div>
            ...
            <div class="bg-white rounded-xl shadow-md overflow-hidden">
                    <div class="p-4 border-b border-gray-100">
                        <h2 class="font-semibold text-gray-700">Budget Requests</h2>
                    </div>
                    <div>
                                        ...
                                        <tr data-request-id="<?php echo (int)$r['request_id']; ?>">
                                            ...
                                            <td data-label="Actions">
                                                <div class="flex flex-wrap gap-2">
                                                    <a class="btn btn-sm btn-outline" href="financial.php?request_id=<?php echo (int)$r['request_id']; ?>">View</a>
  ...
  (function () {
    ...
    if (closeBtn) {
      closeBtn.addEventListener('click', function () {
        window.location.href = 'financial.php<?php echo $programId ? '?program_id=' . (int)$programId : ''; ?>';
      });
    }
    ...
    const submitStatusAjax = async (form) => {
      ...
      try {
        const res = await fetch('financial.php', { method: 'POST', body: fd, credentials: 'same-origin' });
