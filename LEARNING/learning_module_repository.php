...
$modules = [];
$sql = "SELECT * FROM learning_modules WHERE status IN ('approved', 'rejected', 'compliance', 'hold') ORDER BY 
    CASE 
...
<head>
  ...
  <style>
    ...
    .swal2-popup {
      ...
      z-index: 10050 !important; /* Higher than modal z-index */
    }
    
    .swal2-container {
      z-index: 10050 !important; /* Ensure it's above modals */
    }
                    ...
                    <?php foreach ($modules as $module): ?>
                        <div class="card bg-base-100 shadow-md module-card" 
                            ...
                            <div class="card-body">
                                ...
                                <?php elseif ($module['status'] === 'compliance' && !empty($module['compliance_reason'])): ?>
    ...
    function showApprovedModal(moduleData) {
      ...
      document.getElementById('approved-convert-btn').onclick = function() {
        ...
        Swal.fire({
          ...
          if (result.isConfirmed) {
            showNotification('Module converted successfully: ' + moduleData.id, 'success');
          }
        });
      };
