...
if ($edit_exam_data) {
  $assigned_department = (string)($edit_exam_data['department'] ?? '');
  $assigned_roles = (string)($edit_exam_data['roles'] ?? '');
  $exam_duration_minutes = (string)($edit_exam_data['duration'] ?? '');
  $exam_passing_score = (string)($edit_exam_data['passing_score'] ?? '');
} elseif ($module_data) {
  $assigned_department = (string)($module_data['department'] ?? '');
  $assigned_roles = (string)($module_data['roles'] ?? '');
}

if ($exam_duration_minutes === '') {
  $exam_duration_minutes = '60';
}
if ($exam_passing_score === '') {
    ...
    <div class="flex flex-col flex-1 overflow-auto">
                    ...
                    <div class="exam-scroll-container" id="examScrollContainer">
                        ...
                        <div class="space-y-6">
                          ...
                          <div class="form-control">
                            ...
                            <textarea id="examDescription" name="examDescription"
                              class="textarea textarea-bordered h-24"><?php echo $exam_description; ?></textarea>
                          </div>
                          ...
                          <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div class="form-control">
                              <label class="label">
                                <span class="label-text font-semibold text-gray-700">Assigned Department</span>
                              </label>
                              <select id="assignedDepartment" name="assignedDepartment" class="select select-bordered w-full">
                                ...
                                if ($assigned_department !== '' && !in_array($assigned_department, $deptOptions, true)) {
                                  $deptOptions[] = $assigned_department;
                                }
                                foreach ($deptOptions as $dept) {
                                  $selected = $dept === $assigned_department ? 'selected' : '';
                                  $label = ucwords(str_replace(['-', '_'], ' ', $dept));
                                  echo '<option value="' . htmlspecialchars($dept) . '" ' . $selected . '>' . htmlspecialchars($label) . '</option>';
                                }
                                ?>
                              </select>
                            </div>
                            <div class="form-control">
                              <label class="label">
                                <span class="label-text font-semibold text-gray-700">Assigned Roles</span>
                              </label>
                              <input type="text" id="assignedRoles" name="assignedRoles" class="input input-bordered w-full" value="<?php echo htmlspecialchars($assigned_roles); ?>" placeholder="Enter roles, separated by commas">
                            </div>
                          </div>
                          ...
                          <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div class="form-control">
                              <label class="label">
                                <span class="label-text font-semibold text-gray-700">Examination Duration (minutes)</span>
                              </label>
                              <input type="number" min="1" id="examDuration" name="examDuration" class="input input-bordered w-full" value="<?php echo htmlspecialchars($exam_duration_minutes); ?>">
                            </div>
                            <div class="form-control">
                              <label class="label">
                                <span class="label-text font-semibold text-gray-700">Passing Score (%)</span>
                              </label>
                              <input type="number" min="1" max="100" id="passingScore" name="passingScore" class="input input-bordered w-full" value="<?php echo htmlspecialchars($exam_passing_score); ?>">
                            </div>
                          </div>
                        </div>
                      </div>
                      ...
                      <div class="p-6 md:p-8 questions-content">
                        <div class="flex items-center justify-between mb-6">
                          ...
                          <button type="button" id="shuffleQuestions" class="btn btn-outline btn-compact">
                        ...
                        <div class="module-content">
                          ...
                          <div class="space-y-4">
                            ...
                            <div class="form-control">
                              ...
                              <input id="aiQuestionCount" type="number" min="1" max="30" value="5" class="input input-bordered w-24" />
                            </div>
      ...
      <script>
        document.addEventListener('DOMContentLoaded', function() {
          ...
          const moduleData = {
            <?php if ($module_data): ?> '<?php echo $module_data['id']; ?>': {
          ...
          let aiLastQuestionType = 'multiple_choice';
          ...
          const aiGenerate = () => {
            ...
            const questionType = aiQuestionTypeSelect ? aiQuestionTypeSelect.value : 'multiple_choice';
            ...
            if (!Number.isFinite(questionCountNum) || questionCountNum < 1 || questionCountNum > 30) {
              aiShowError('Please enter a valid number of questions (1-30)');
              return;
            }
          ...
          function saveFormState() {
            const formState = {
              ...
              assignedRoles: document.getElementById('assignedRoles')?.value || '',
          ...
          function loadFormState() {
            ...
            try {
              ...
              if (document.getElementById('assignedDepartment')) {
                document.getElementById('assignedDepartment').value = formState.assignedDepartment || document.getElementById('assignedDepartment').value;
              }
              if (document.getElementById('assignedRoles')) {
                document.getElementById('assignedRoles').value = formState.assignedRoles || document.getElementById('assignedRoles').value;
              }
              if (document.getElementById('examDuration')) {
                document.getElementById('examDuration').value = formState.examDuration || document.getElementById('examDuration').value;
              }
              if (document.getElementById('passingScore')) {
                document.getElementById('passingScore').value = formState.passingScore || document.getElementById('passingScore').value;
              }
        ...
        `;
          ...
          if (shuffleQuestionsBtn) {
            shuffleQuestionsBtn.addEventListener('click', function() {
              const items = Array.from(questionsContainer.querySelectorAll('.question-item'));
              if (items.length <= 1) {
                showToast('Not enough questions to shuffle', 'info');
                return;
              }
              ...
              shuffled.forEach((el) => fragment.appendChild(el));
              questionsContainer.appendChild(fragment);
              updateQuestionNumbers();
              saveFormState();
              showToast('Questions order shuffled', 'success');
            });
          }
        ...
        `;
          ...
          createExamBtn.addEventListener('click', function(e) {
            ...
            const assignedDepartmentInput = document.getElementById('assignedDepartment');
            const assignedRolesInput = document.getElementById('assignedRoles');
            ...
            const chosenDepartment = assignedDepartmentInput ? assignedDepartmentInput.value.trim() : '';
            const chosenRoles = assignedRolesInput ? assignedRolesInput.value.trim() : '';
            let durationMinutes = durationInput ? parseInt(durationInput.value, 10) : 0;
            let passingScoreValue = passingInput ? parseInt(passingInput.value, 10) : 0;

            if (!chosenDepartment) {
              showToast('Please select an assigned department', 'error');
              if (assignedDepartmentInput) assignedDepartmentInput.focus();
              createExamBtn.classList.remove('btn-loading');
              createExamBtn.disabled = false;
              return;
            }
            ...
            if (!chosenRoles) {
              showToast('Please enter assigned roles', 'error');
              if (assignedRolesInput) assignedRolesInput.focus();
              createExamBtn.classList.remove('btn-loading');
              createExamBtn.disabled = false;
              return;
            }
            ...
            if (!Number.isFinite(passingScoreValue) || passingScoreValue <= 0 || passingScoreValue > 100) {
              passingScoreValue = 70;
            }
            ...
            const examData = {
              ...
              department: chosenDepartment || moduleDepartment,
              roles: chosenRoles || moduleRoles,
              status: 'pending',
              questions: [],
              total_points: 0,
