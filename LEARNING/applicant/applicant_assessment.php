...
$applicantLastName = trim((string)($_SESSION['applicant_last_name'] ?? ''));
$applicantFirstName = trim((string)($_SESSION['applicant_first_name'] ?? ''));
$applicantMiddleInitial = trim((string)($_SESSION['applicant_middle_initial'] ?? ''));

if ($applicantName === '' && ($applicantLastName !== '' || $applicantFirstName !== '')) {
  $mid = $applicantMiddleInitial !== '' ? mb_substr($applicantMiddleInitial, 0, 1, 'UTF-8') : '';
  if ($applicantLastName !== '' && $applicantFirstName !== '') {
    if ($mid !== '') {
      $applicantName = $applicantLastName . ', ' . $applicantFirstName . ' ' . mb_strtoupper($mid, 'UTF-8') . '.';
    } else {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['context_submit'])) {
  ...
  $middle = trim((string)($_POST['middle_initial'] ?? ''));
  $dept = trim((string)($_POST['department'] ?? ''));
  $role = trim((string)($_POST['role'] ?? ''));

  $normalizedName = '';
  if ($last !== '' && $first !== '') {
    $mid = $middle !== '' ? mb_substr($middle, 0, 1, 'UTF-8') : '';
    if ($mid !== '') {
      $normalizedName = $last . ', ' . $first . ' ' . mb_strtoupper($mid, 'UTF-8') . '.';
    } else {
      $normalizedName = $last . ', ' . $first;
    }
  }

  if ($normalizedName !== '' && $dept !== '' && $role !== '') {
    $_SESSION['applicant_name'] = $normalizedName;
    $_SESSION['applicant_last_name'] = $last;
    $_SESSION['applicant_first_name'] = $first;
    $_SESSION['applicant_middle_initial'] = $middle !== '' ? mb_substr($middle, 0, 1, 'UTF-8') : '';

    $applicantName = $normalizedName;
    $applicantLastName = $last;
    $applicantFirstName = $first;
    $applicantMiddleInitial = $middle !== '' ? mb_substr($middle, 0, 1, 'UTF-8') : '';
                ...
                <form method="POST" class="space-y-4">
                  ...
                  <div class="grid grid-cols-1 gap-4">
                    <div class="form-control">
                      ...
                      <input class="input input-bordered" type="text" name="last_name" required value="<?php echo htmlspecialchars($applicantLastName); ?>" placeholder="Enter your last name">
                    </div>
                    <div class="form-control">
                      <label class="label"><span class="label-text font-medium">First Name</span></label>
                      <input class="input input-bordered" type="text" name="first_name" required value="<?php echo htmlspecialchars($applicantFirstName); ?>" placeholder="Enter your first name">
                    </div>
                    <div class="form-control">
                      <label class="label"><span class="label-text font-medium">Middle Initial</span></label>
                      <input class="input input-bordered" type="text" name="middle_initial" maxlength="2" value="<?php echo htmlspecialchars($applicantMiddleInitial); ?>" placeholder="Optional">
                    </div>
                  </div>
                  <div class="form-control">
